	...
	refresh (omitChanges) {
		...
		return this.action('refresh', e => mdbc.findOne({_id}).then(res => {
			...
			if (omitChanges) {
				...
				this.data.merged = extend({}, this.data.written);
			} else {
				this.data.merged = extend({}, this.data.written, this.data.changes || {});
			}
		...
		if (info.method === 'insert') {
			...
			/*
				Dafuk
				{
					updateOne: {
						filter: {
							_id: { _bsontype: 'ObjectID', id: <Buffer 59 ce 4e 81 33 2f a1 2f e0 70 46 f6> }
						},

						update: {
							'$set': { 'names.last': 'Snigelfors' }
						},
