...
class Document extends Pluggable {
	...
	get (includeChanges, key, sentinel) {
		...
		var data = dotnot ? this.dataDotted : this.dataRegular;
		...
		// then we could return dottedData[key] for some
		// better performance?
	...
	set (asWritten, key, val) {
		...
		if (asWritten || changes._id) {
			...
			if (data.written) {
				shallowCopy(data.written, changes);
			} else {
		...
		// Handle case when the user is adding changes to those which we already have.
			...
			shallowCopy(data.changes, changes);
		}
	...
	refresh (omitChanges) {
		...
		return this.action('refresh', e => mdbc.findOne({_id}).then(res => {
			this.dataDotted.written = obj2dot(res);

			if (omitChanges) {
				this.dataDotted.changes = null;
			}
	...
	_pullChanges () {
		...
		if (data.changes) {
			shallowCopy(this.dataDotted.written, this.dataDotted.changes);
		}
