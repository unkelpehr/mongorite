...
const set = require('./Document.set');
...
class Document extends Pluggable {
	constructor (collection) {
		...
		this.set = require('./Document.set');
		this.get.changes = () => {
			return this.data.changes;
		};

		this.unset.change = (key) => {
			return this.unset(key, set.CONST.UNSET_CHANGE);
		};
	...
	get (includeChanges, key, sentinel) {
		const dotnot = sentinel === set.CONST.GET_DOTNOT;
		...
		if (includeChanges && !data.merged) {
			...
			transform2(data.merged, data.written, data.merged['@@dotnot']);
			transform2(data.merged, data.changes, data.merged['@@dotnot']);
		}
	...
	unset (key, sentinel) {
		...
		const change = sentinel === set.CONST.UNSET_CHANGE;
	...
	refresh (omitChanges) {
		...
		return this.action('refresh', e => mdbc.findOne({_id}).then(res => {
			...
			this.set(set.CONST.SET_WRITE, res);
	...
	_pullChanges () {
		...
		if (data.changes) {
			...
			this.set(set.CONST.SET_MERGE_WRITE, data.changes)
