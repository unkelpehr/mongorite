...
class Collection extends Pluggable {
	 ...
	 * @param {Boolean=} throwOnError if TRUE; throws if one or more write error occurs.
	 * @param {Mongorite.Document=} doc
	 * @param {Mongorite.Document=} _baseError
	 * @return {Object} `this`
	 */
	save (force, throwOnError, doc, _baseError) {
		...
		const data = {
			...
			force: !!force,
			mode: arguments.length >= 2 ? 'single' : 'multi'
		};
		...
		const docs = data.mode === 'single' ? [doc] : this;

		docs.forEach(collectOps);
		...
		_baseError = (_baseError instanceof Error) ? _baseError : new Error();
		...
		return this
			.action('save', data, e =>
				mdbc.bulkWrite(e.data._operations, e.data.options).then(_res => {
					...
					const errors = _res.getWriteErrors();

					// Build `res.erroredDocs` map.
					for (let i = 0; i < errors.length; ++i) {
						const writeError = errors[i];
						const document = docs[writeError.index];

						writeError.document = document;
						res.erroredDocs[document] = writeError;
					}
					...
					if (errors.length > 0) {
						...
						const stack = _baseError.stack.replace(/.+\n?/i, '');
						const message = (
							data.mode === 'single'
								? `${errors[0].errmsg} (${errors[0].document.id})`
								: `${errors.length} / ${docs.length} documents could not be saved:` + errors.map(err => `\n${err.document.id}: ${err.errmsg}`).join('\n')
						);

						Object.defineProperties(res.error, {
							name: {get: () => 'WriteError'},
							message: {get: () => message},
							stack: {get: () => `WriteError: ${message}\n${stack}`},
							documents: {
								enumerable: true,
								value: res.erroredDocs
							}
						});
					}
