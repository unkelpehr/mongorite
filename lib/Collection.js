...
class Collection extends Pluggable {
	...
	save () {
		...
		const after = this.dispatchEvent('save', full_ops);
	...
	push () {
		...
		for (i = 0; i < arguments.length; ++i) {
			...
			for (j = 0; j < arg.length; ++j)  {
				if (arg[j]) {
					this[this.length++] = arg[j].isMongoriteDocument ? arg[j] : (new Document(this)).set(arg[j]);
				}
			}
		}
		console.timeEnd('dafuk?');
