...
function hr2ms (hr) {
	return ((hr = process.hrtime(hr)), (hr[0] * 1E3) + (hr[1] / 1E6));
}
...
class Collection {
	constructor (db) {
		...
		hide(this, '_refs', {
			...
			ajv: {
				schema: null,
				options: {allErrors: true}, // https://www.npmjs.com/package/ajv#options
				validate: null
			}
		});
	...
	get (key) {
		...
		this.each(doc => vals.push(doc.get(key)));
	...
	clone (keepDocuments) {
		...
		const ignore = {query: true};

		for (let i = 0; i < props.length; ++i) {
			if ((keepDocuments || isNaN(props[i])) && !ignore[props[i]]) {
